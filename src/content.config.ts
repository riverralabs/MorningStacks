...
import { publishGateErrors } from './lib/content-model';
...
const articles = defineCollection({
  ...
  schema: ({ image }) =>
    z
      .object({
        type: z.enum(['review', 'roundup', 'briefing', 'explainer']),
        status: visibility,
        ...
        lastTested: z.coerce.date().nullish(),
        testMethod: z.string().optional(),
        ourPick: z.string().optional(),
      ...
      .superRefine((data, ctx) => {
        for (const error of publishGateErrors(data)) {
          ctx.addIssue({
